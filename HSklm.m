function [As, av, cs, cv, usg, vsg, uvg, vvg] = HSklm(beta_s, aCoor_s, cCoor_r, nvec_r, tvec_r, pl_s)
    ...
    % Ensure cv is properly calculated
    cv = sum(cvv);
    %bv = dot(-U_infv, tvec_r(:, 1)) + dot(-U_infv, tvec_r(:, end)); % Assemble Kutta condition vector
end